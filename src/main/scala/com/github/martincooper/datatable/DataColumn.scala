...
class DataColumn[T: TypeTag](columnName: String, columnData: Iterable[T])
  ...
  override def add(value: DataValue): Try[GenericColumn] = {
    validateType("add", value).flatMap { typedValue =>
      checkAndCreateNewColumn(
        () => IndexedSeqExtensions.addItem(data, typedValue)
      )
    }
  }
  ...
  override def insert(index: Int, value: DataValue): Try[GenericColumn] = {
    validateType("insert", value).flatMap { typedValue =>
      checkAndCreateNewColumn(
        () => IndexedSeqExtensions.insertItem(data, index, typedValue)
      )
    }
  }
  ...
  override def replace(index: Int, value: DataValue): Try[GenericColumn] = {
    validateType("replace", value).flatMap { typedValue =>
      checkAndCreateNewColumn(
        () => IndexedSeqExtensions.replaceItem(data, index, typedValue)
      )
    }
  }
  ...
  private def checkAndCreateNewColumn(
      transformData: () => Try[IndexedSeq[T]]
  ): Try[DataColumn[T]] = {
    transformData().map { modifiedData =>
      new DataColumn[T](name, modifiedData)
    }
  }
